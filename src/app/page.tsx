...
function HomeContent() {
  ...
  // ── Search helper: fetch from Careerjet ──────────────────────
  const searchCareerjet = useCallback(async (query: string, loc: string, city?: string) => {
    // Require both search term and a specific country
    if (!query || !loc) { setLoading(false); return; }
    try {
      setLoading(true);
      const params = new URLSearchParams({ q: query });
      if (loc) params.set("country", loc);
      if (city) params.set("city", city);
      if (user?.id) params.set("userId", user.id);

      const res = await fetch(`/api/careerjet-search?${params.toString()}`);
      if (!res.ok) throw new Error("Careerjet API error");
      const data = await res.json();
      if (data.jobs?.length > 0) {
        setJobs(data.jobs);
        setVisibleCount(20);
      } else {
        throw new Error("No Careerjet results");
      }
    } catch {
      // Fallback: existing DB jobs
      const res = await fetch("/api/jobs?fetch_all=true");
      if (res.ok) {
        const data = await res.json();
        if (data.jobs?.length > 0) {
          const mapped = mapDbJobs(data.jobs);
          setJobs(mapped);
          return;
        }
      }
  ...
  // ── Map DB job rows to frontend interface ────────────────────
  const mapDbJobs = useCallback((dbJobs: any[]): Job[] => {
    return dbJobs.map((j: any, i: number) => {
  ...
  useEffect(() => {
    ...
    async function loadJobs() {
      try {
        ...
        // ── If a search query is in the URL, use Careerjet directly ──
        const urlQuery = sp.get("q")?.trim();
        if (urlQuery) {
          const params = new URLSearchParams({ q: urlQuery });
          const loc = sp.get("country");
          if (loc) params.set("country", loc);
          if (user?.id) params.set("userId", user.id);
          ...
          // Fallback: DB search
          if (!cancelled) {
            const res = await fetch(`/api/jobs?search=${encodeURIComponent(urlQuery)}&limit=200`);
            if (res.ok) {
              const data = await res.json();
              if (data.jobs?.length > 0) {
                setJobs(mapDbJobs(data.jobs));
              } else {
                setJobs(fallbackJobs);
              }
            } else {
        ...
        if (user && profile?.role === "employee") {
          // ── Logged-in employee: use professional headline + DB ──
          // Fetch their headline from employee_profiles
          const sup = createClient();
          const { data: empProfile } = await sup
            .from("employee_profiles")
            .select("headline, location")
            .eq("id", user.id)
            .single();

          let searchTerm = empProfile?.headline || "";
          if (!searchTerm) {
            // Fall back: show all active jobs from DB
            const res = await fetch("/api/jobs?fetch_all=true");
            if (res.ok) {
              const data = await res.json();
          ...
          // Try Careerjet first with their headline + location
          try {
            const params = new URLSearchParams({ q: searchTerm, userId: user.id });
            const res = await fetch(`/api/careerjet-search?${params.toString()}`);
            if (res.ok) {
              const data = await res.json();
              if (!cancelled && data.jobs?.length > 0) {
          ...
          // Fallback: DB jobs filtered by headline
          if (!cancelled) {
            const res = await fetch(`/api/jobs?search=${encodeURIComponent(searchTerm)}&limit=200`);
            if (res.ok) {
              const data = await res.json();
              if (data.jobs?.length > 0) {
                setJobs(mapDbJobs(data.jobs));
          ...
          // ── Logged-out: use IP-based location + Careerjet ──
          try {
            // 1. Get geo location
            ...
            if (geoRes.ok) {
              ...
              resolvedCountry = geo.country || "United States";
            }

            // 2. Show jobs from the user's country — auto-select it in the dropdown
            if (resolvedCountry) {
              updateFilter("country", resolvedCountry);
            }
            const params = new URLSearchParams({ q: "jobs" });
  ...
  // ── Commit draft search + execute ───────────────────────────
  const doSearch = useCallback((draft: string, loc: string, cty?: string) => {
    updateFilter("search", draft);
    updateFilter("country", loc);
    searchCareerjet(draft, loc, cty);
  }, [updateFilter, searchCareerjet]);

  const filteredJobs = useMemo(() => {
    // Search is already handled by Careerjet API — only filter by non-search criteria here.
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-dark-bg text-gray-900 dark:text-gray-100 transition-colors">
            ...
            <div className="search-glass-3d rounded-2xl p-2 flex flex-col sm:flex-row gap-2">
              <div className="flex-1 relative">
                ...
                <input
                  ...
                  onKeyDown={(e) => { if (e.key === "Enter" && searchDraft && countryDraft) { e.preventDefault(); doSearch(searchDraft, countryDraft, filters.city); } }}
              ...
              <div className="relative">
                ...
                <select
                  ...
                  style={{ background: "rgba(255,255,255,0.06)" }}
                >
                  <option value="" className="bg-gray-900" disabled>Select a country</option>
                  {countries.map((c) => (
              ...
              <div className="relative">
                <input
                  ...
                  onKeyDown={(e) => { if (e.key === "Enter" && searchDraft && countryDraft) { e.preventDefault(); doSearch(searchDraft, countryDraft, filters.city); } }}
                    className="w-36 pl-3 pr-2 py-2.5 rounded-xl border border-white/10 bg-white/08 text-white placeholder-white/35 focus:border-white/30 text-sm outline-none transition-colors"
                    style={{ background: "rgba(255,255,255,0.06)" }}
                  />
                </div>
              <button
                onClick={() => doSearch(searchDraft, countryDraft, filters.city)}
                disabled={!searchDraft || !countryDraft}
                className="btn-beam bg-primary text-white px-7 py-2.5 rounded-xl font-semibold hover:bg-primary-dark disabled:opacity-50 disabled:cursor-not-allowed transition-colors text-sm shadow-lg shadow-blue-900/40"
              >
                {t("search.button")}
              </button>
            </div>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <div className="flex gap-6">
          {/* ── Sidebar Filters (Desktop) — only for logged-in users ── */}
          {user && <aside className="hidden md:block w-56 shrink-0">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
            ...
            <div>
              ...
              <ul className="space-y-1.5 text-xs">
                ...
                <li><Link href="/contact" className="hover:text-white dark:hover:text-gray-300">{t("footer.contact")}</Link></li>
...
function ActiveFilterTags({
  ...
  if (filters.country) tags.push({ label: `${t("filter.country")}: ${filters.country}`, onRemove: () => onRemove("country") });
